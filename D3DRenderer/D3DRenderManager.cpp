...
void D3DRenderManager::Update(float deltaTime)
{
	...
	{
		if (SkeletalMeshModel.first->m_IsUIMove == true)
		{
...
void D3DRenderManager::ConstantBuffUpdate()
{
	...
	// 그림자
	{
		...
		Vector3 forward = m_View.Forward() * m_ShadowForwardDistFromCamera;

		// 그림자 View 행렬, Projection 행렬 만들기
		m_ShadowProjection = XMMatrixPerspectiveFovLH(XM_PIDIV4, SHADOWMAP_SIZE / SHADOWMAP_SIZE, 2000.f, 2000000.f);
