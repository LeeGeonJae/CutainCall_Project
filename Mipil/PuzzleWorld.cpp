...
void PuzzleWorld::Initialize()
{
	...
	auto ReadyClickPopUpFunctor = std::make_shared <UIReadyClickPopUpFunctor>(); // 요기
	...
	if (WorldManager::GetInstance()->IsHostServer())
	{
		...
		ReadyClickPopUpFunctor->SetPlayer(hostPlayer); // 요기
	}
	else
	{
		ReadyClickFunctor->SetPlayer(guestPlayer);
		ReadyClickPopUpFunctor->SetPlayer(guestPlayer); // 요기
	}
